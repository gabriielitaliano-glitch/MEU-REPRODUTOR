...
<nav>
  <button onclick="showSection('inicio')" id="btn-inicio">Início</button>
  ...
  <button onclick="showSection('relatorios')" id="btn-relatorios">Relatórios</button>
</nav>

<section id="inicio">
  <h2>Bem-vindo(a)!</h2>
  <p>Painel inicial de demonstração do manejo ovino/caprino.</p>
</section>
...
<section id="rebanho">
  ...
  <label>Identificação: <input id="new-id"></label><br>
  <label>Espécie:
  ...
  <label>Raça: <input id="new-raca"></label><br>
  <label>Sexo:
    <select id="new-sexo">
  ...
  <label>Observações: <input id="new-obs"></label><br><br>
...
<footer style="text-align:center; padding:10px; background:#1e293b; color:white;">
  Criado por: Mariana, Alan, Giovanna, Flávia, Gabriele
</footer>
...
<script>
  ...
  function addAnimal(){
    const a={
      ...
      nascimento: document.getElementById('new-nasc').value||'—',
  ...
  function updateAnimals(){
    ...
    db.animals.forEach(a=>{
      lista.innerHTML+=`<div class="item"><b>${a.id}</b> — ${a.especie} ${a.raca} (${a.sexo}) | Nasc: ${a.nascimento} | Obs: ${a.obs}</div>`;
    });
  }
  ...
  function updateReports(){
    document.getElementById('report').innerHTML=`<p>Total de animais: <b>${db.animals.length}</b></p>
      ...
      <p>Total de usuários demo: <b>${defaultUsers.length}</b></p>`;
  }
